...
void renderPlane(btVector3 a, int type)
{
    ...
    /*
     ...
     if (once == 5){
     Sol->getMotionState()->getWorldTransform(t);
     qa = t.getRotation();
     ...
     Sol->getMotionState()->setWorldTransform(t);
     }
...
void display(void){
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        for (int j = dynamicsWorld->getNumCollisionObjects() - 1; j >= 0; j--) //sphere,sol,mur
        {
            ...
            if(body->getCollisionShape()->getShapeType()!=SPHERE_SHAPE_PROXYTYPE){
                if(j > 1)//que ce n'est pas le sol
                    renderPlane(btVector3(angleX,angleY,angleZ),2);
                else
                    renderPlane(btVector3(angleX,angleY,angleZ),1);
            }
            else if(body->getCollisionShape()->getShapeType()==SPHERE_SHAPE_PROXYTYPE)
                renderSphere(body);
        }
        
        cout << "plopDIPLAY"<<endl;
    }
    glPopMatrix();
    //on affiche
...
void init_bull(btVector3 angle){ //ici passer le Vec3f
    ...
    //Sphere
    {
        ...
        //startPoint
        startTransform.setOrigin(btVector3(2, 7, -2));
