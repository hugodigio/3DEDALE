...
double isVectorsCollinear(Vec4f segment_1, Vec4f segment_2)
{
...
void normalizedLines (vector<Vec4f> &lines) {
 ...
 * 
 * */
bool mixLines (vector<Vec4f> &lines) {
 ...
 * 
 * */
bool deleteLineMin (vector<Vec4f> &lines) {
    int fontFace = FONT_HERSHEY_COMPLEX_SMALL;
    double fontScale=3.5;
    int cpt = 0;
    int espacement = 30;
 ...
 * 
 * */
bool mixAlignedLines (vector<Vec4f> &lines) {
...
bool orthoLines(vector<Vec4f> &lines) {
	
	int precision = 5; // precision de l'angle
	bool changement = false;
	vector<Vec4f> linesResult;
...
vector<Vec4f> findWalls(Mat src) {
